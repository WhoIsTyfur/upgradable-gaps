...
public final class SelfTest {
    ...
    public void started(FMLServerStartedEvent event) {
        ...
        EntityPlayer player = new FakePlayer(world, "selftest");
        StringBuilder failures = new StringBuilder();

        ContainerWorkbench menu = fill(player, 8, 1, Item.appleGold);
        ...
        menu = fill(player, 16, 2, Item.appleGold);
        ...
        menu = fill(player, 8, 1, Item.appleGold);
        ...
        menu = fill(player, 1, 1, Item.appleRed);
        ItemStack vanilla = menu.getSlot(0).getStack();
        expect(failures, "vanilla golden apple", vanilla != null && vanilla.itemID == Item.appleGold.itemID && vanilla.getItemDamage() == 0);
    ...
    private static ContainerWorkbench fill(EntityPlayer player, int ingots, int centerCount, Item center) {
        ...
        for (int slot = 1; slot <= 9; slot++) {
            menu.getSlot(slot).putStack(slot == CENTER ? new ItemStack(center, centerCount, 0) : new ItemStack(Item.ingotGold, ingots));
        }
        return menu;
    }

    private static boolean isNotchApple(ItemStack stack, int count) {
        return stack != null && stack.itemID == Item.appleGold.itemID && stack.getItemDamage() == 1 && stack.stackSize == count;
    }
    ...
    private static int count(EntityPlayer player, Item item, int damage) {
        ...
        for (ItemStack stack : player.inventory.mainInventory) {
            if (stack != null && stack.itemID == item.itemID && stack.getItemDamage() == damage) {
